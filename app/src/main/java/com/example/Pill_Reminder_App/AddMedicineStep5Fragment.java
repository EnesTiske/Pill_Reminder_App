...
public class AddMedicineStep5Fragment extends Fragment {
    ...
    private List<TimeDose> timeDoseList = new ArrayList<>();
    ...
    private void addTimeDoseRow(String time, int dose) {
        ...
        tvDose.setOnClickListener(v -> {
            ...
            builder.setItems(items, (dialog, which) -> tvDose.setText(items[which]));
            builder.show();
        });

        timesContainer.addView(row);
        timeDoseList.add(new TimeDose(time, dose));
    }

    // Saat ve adet bilgisini tutmak için yardımcı sınıf
    private static class TimeDose {
        String time;
        int dose;
        TimeDose(String time, int dose) {
            this.time = time;
            this.dose = dose;
        }
    }
